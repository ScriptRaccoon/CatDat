...
(
	...
	TRUE,
	'Consider the diagram of smooth maps $X + X \overset{p}{\to} E \overset{r}{\to} X$. Since $p$ is assumed to be an epimorphism, it has dense image. We first claim that in fact $p$ also has closed image and therefore is surjective. To see this, consider a sequence $(e_n)$ in the image of $p$ converging to $e \in E$. Then there is either a subsequence whose elements are in the image of $p\circ i_1$, or a subsequence whose elements are in the image of $p\circ i_2$. In the first case, say $e_{n_j} = p(i_1(x_{n_j}))$. Then $r(e_{n_j}) = x_{n_j}$, so $x_{n_j} \to r(e)$. It follows that $e = \lim e_{n_j} = \lim p(i_1(x_{n_j})) = p(i_1(r(e)))$. The second case is similar.
<p>Now, since the pushforward maps of tangent spaces compose to the identity, we see that $p$ must be a local immersion and $r$ must be a submersion.  Also, since the fibers of $r$ have one or two points each, we see that the dimension of $E$ must locally be the same as the dimension of $X$.  This implies that in fact $p$ and $r$ are local diffeomorphisms.  Therefore, the cardinality of the fiber of $r$ is locally constant.  Thus, if $U$ is the subset of $X$ where $r$ has fiber of a single point, with the subspace topology, then $U$ is a clopen submanifold of $X$ which serves as the equalizer of $p \circ i_1$ and $p \circ i_2$.'
),
(
	'Man',
	'small',
