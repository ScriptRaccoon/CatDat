CREATE TABLE categories (
    ...
    FOREIGN KEY (name, type) REFERENCES structures (name, type) ON DELETE CASCADE
);

CREATE VIEW categories_view AS
    SELECT
        s.name, s.name, s.notation, s.description, s.nlab_link,
        c.objects, c.morphisms, d.dual_structure as dual_category
    FROM categories c
    INNER JOIN structures s ON s.name = c.name
    LEFT JOIN dual_structures d ON d.structure = c.name AND d.type = c.type
;
