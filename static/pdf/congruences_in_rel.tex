...
Suppose that under the equivalence relation on $P(X)$, we have $S \sim \varnothing$.  Then for any $x \in S$, we have $\{ x \} = \{ x \} \cup \varnothing \sim \{ x \} \cup S = S \sim \varnothing$.  Using the fact that $i_*$ preserves unions for the inverse direction, this shows that $S \sim \varnothing$ if and only if $\{ x \} \sim \varnothing$ for every $x \in S$.

Suppose that $r_*(\{ x \}) = \{ e \}$.  We then claim that $\{ x \} \not\sim \varnothing$.  If so, then there would have to be some subset of $E$ which maps to $(\varnothing, \{ x \})$, and similarly to previous steps, this implies that this subset of $E$ would have to be a singleton $\{ f_1 \}$.  Similarly, $(\{ x \}, \varnothing)$ would have to be the image of a singleton $\{ f_2 \}$.  But then $i_*(\{ f_1, f_2 \}) = (\{ x \}, \{ x \}) = i_*(\{ e \})$, contradicting the injectivity of $i_*$.
