...
The same proof works for regular subobjects.
:::
...
Since $U(\colim_i f_i)$ is a monomorphism in $\D$ and $U$ is faithful, it follows that $\colim_i f_i$ is a monomorphism in $\C$.
:::
...
::: Proof
For any objects $X, Y, Z$ of $\C$ we have natural isomorphisms
...
::: Proof
The forgetful functor $\C / P \to \C$ is fully faithful; it has right adjoint ${-} \times P$; and it preserves binary products (in fact all inhabited limits). Hence, Lemma 5 applies.
:::
...
is a composition of faithful functors, hence faithful.
:::
